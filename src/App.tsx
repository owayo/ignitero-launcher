...
function App() {
  ...
  useEffect(() => {
    ...
    const unlisten = appWindow.onFocusChanged(async ({ payload: focused }) => {
      ...
      if (focused) {
        ...
        try {
          const [visible, pickerState, terminalPickerState] = await Promise.all(
            [
              appWindow.isVisible(),
              invoke<WindowState | null>('get_window_state', {
                ...
                console.error(
                  'Failed to get editor picker window state:',
                  error,
                );
                return null;
              }),
              invoke<WindowState | null>('get_window_state', {
                ...
                console.error(
                  'Failed to get terminal picker window state:',
                  error,
                );
                return null;
              }),
          ...
          console.log('[main-window] blur state', {
            visible,
            pickerState,
            terminalPickerState,
          });
          ...
          if (
            pickerState?.visible ||
            pickerState?.focused ||
            terminalPickerState?.visible ||
            terminalPickerState?.focused
          ) {
            console.log(
              'Main window lost focus but picker is visible/focused; keeping it open.',
            );
            return;
          }
          ...
          if (!visible) {
            console.log(
              'Main window hidden; requesting picker windows close if they exist.',
            );
            invoke('close_editor_picker_window').catch((error) => {
              console.error('Failed to close editor picker window:', error);
            });
            invoke('close_terminal_picker_window').catch((error) => {
              console.error('Failed to close terminal picker window:', error);
            });
          } else {
            console.log(
              'Main window lost focus but is still visible; keeping picker windows open.',
            );
          }
        } catch (error) {
          console.error(
  ...
  // ターミナル選択ウィンドウを開く
  const handleOpenTerminalPickerWindow = useCallback(
    async (item: DirectoryItem) => {
      try {
        console.log('Opening terminal picker window for:', item.path);
        await invoke('open_terminal_picker_window', {
          directoryPath: item.path,
        });
        await invoke('hide_window');
      } catch (error) {
        console.error('Failed to open terminal picker window:', error);
      }
    },
    [],
  );
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent) => {
      ...
      if (e.key === 'ArrowDown') {
        ...
        if (!isAppItem(item) && !isCommandItem(item)) {
          if (e.metaKey) {
            // Command+→: ターミナル選択ウィンドウを開く
            handleOpenTerminalPickerWindow(item);
          } else {
            // →: デフォルトターミナルで開く
    ...
    [
      ...
      handleOpenTerminalPickerWindow,
    ],
  );
                      ...
                      >
                        ...
                        <div
                          style={{
                            ...
                            padding: '2px 8px',
                            borderRadius: 4,
                            border: '1px solid rgba(71, 120, 255, 0.4)',
                            background: 'rgba(255, 255, 255, 0.8)',
                            fontSize: 11,
                            color: 'rgba(60, 60, 70, 0.8)',
                          }}
