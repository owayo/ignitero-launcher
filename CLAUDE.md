...
- **計算機能**: 検索欄に計算式を入力すると結果を表示（Enterでクリップボードにコピー）
...
- **`calculator.ts`** - 安全な計算式評価モジュール
  - 再帰下降パーサーによる四則演算（+, -, *, /, %）
  - 括弧、小数点、単項演算子のサポート
  - `eval()`を使用しないセキュアな実装
  - ユニットテスト: `calculator.test.ts`（35テスト）
...
2. **計算機能**:
   - 検索欄に計算式を入力すると自動的に結果を表示
   - **再帰下降パーサー**: Lexer（字句解析）→ Parser（構文解析）
   - 対応演算子: `+` `-` `*` `/` `%`
   - 演算子優先順位: 乗除剰余 > 加減
   - 括弧によるグループ化、単項演算子（+/-）のサポート
   - useMemoによる計算結果のメモ化
   - `Enter`キーでクリップボードにコピー（Clipboard API）
   - 0除算、0剰余のエラーハンドリング
...
### キーボードナビゲーション
- `↑` `↓`キーで項目選択
- `Enter`キーで起動（計算結果表示時はクリップボードにコピー）
- `→`キーでディレクトリをターミナルで開く
- `Escape`キーでウィンドウを閉じる
