...
現在、Option+Spaceで検索窓を呼び出した際、日本語入力モードのまま表示されることがあります。

**原因**：
- macOSの「書類ごとに入力ソースを自動的に切り替える」設定が有効な場合、ウィンドウ表示時に以前の入力ソースが自動復元されます
- アプリ側で英字入力への強制切り替えを実装していますが、タイミングの問題でmacOS側の自動復元と競合することがあります

**回避策**：
1. **一時的な対処**: 検索窓が表示された後、英数キーを押して英字入力モードに切り替える
2. **根本的な対処**: macOSのシステム設定で「書類ごとに入力ソースを自動的に切り替える」をオフにする
   - システム設定 → キーボード → 入力ソース → 「書類ごとに入力ソースを自動的に切り替える」のチェックを外す

> この問題は将来のバージョンで改善予定です（再フォーカス時の遅延処理やメインスレッド保証の強化）
