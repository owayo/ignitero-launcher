...
Sources/
  ...
  IgniteroLauncher/         # 実行可能ターゲット (@main エントリ)
    IgniteroApp.swift
Tests/
  IgniteroCoreTests/        # 1005テスト (Swift Testing)
.backup/                    # Tauri v2 旧実装 (参照用)
```

補足（2026-08-09）: Warp 0.2026.07.01.09.21.01 は `sdef /Applications/Warp.app` がエラー -192 で AppleScript 辞書を取得できないため、引き続き `.command` ファイル方式を維持する。Terminal.app 2.15 / iTerm2 3.6.11 / Ghostty 1.3.1 / cmux 0.64.22 はローカル辞書で AppleScript 対応を確認済み。
